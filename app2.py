...
def categorize_file(filename: str) -> str:
    ...
    elif "(fito)" in filename_lower: return "Phytosanitary Restriction (FITO)" # <--- NAUJA KATEGORIJA
...
def _process_fito_file(df: pd.DataFrame, filename: str, category: str) -> List[Dict]:
    """
    SPECIALIZUOTA LOGIKA: (FITO) failams.
    ...
    # Fallback: jei neradome, naudojame 0 stulpelį
    if header_idx == -1:
        header_idx = 0
        code_col_idx = 0

    # Pagal užduotį: A=0, B=1, C=2
    # Jei kodas rastas 0 stulpelyje, B yra 1, C yra 2.
    idx_b = 1
    idx_c = 2

    # Jei headeris buvo rastas pvz. 1 stulpelyje, viską pastumiame?
    # Užduotis sakė "A stulpelis", kas reiškia absoliutus 0.
    # Tad naudojame absoliučius indeksus duomenų traukimui, nebent kodas rastas ne A.
    if code_col_idx != 0:
        # Jei algoritmas rado kodą kitur, prisitaikome reliatyviai
...
def load_excel_csv_data(data_folder_str: str) -> List[Dict]:
    ...
    for file_path in all_files:
        try:
            ...
            elif "(fito)" in filename_lower: # <--- NAUJAS MARŠRUTAS (FITO)
...
def search_codes(user_input: str, all_codes: List[Dict]) -> List[Dict]:
    ...
    for entry in all_codes:
        ...
        if "dual use" in cat or "dvejopo naudojimo" in cat or "(du)" in src:
            ...
            # Standartinė hierarchinė paieška (tinka ir SA, ir FITO)
...
def extract_tags_from_matches(matches: List[Dict]) -> str:
    ...
    for m in matches:
        ...
        elif "(fito)" in src: tags.add("FITO") # <--- Naujas tagas
